...
CREATE TABLE `Post` (
  ...
  `postType` enum('CLUB_REVIEW','CLUB_ANNOUNCEMENT') NOT NULL,
  `createdAt` datetime DEFAULT CURRENT_TIMESTAMP,
  `images` json DEFAULT NULL,
  PRIMARY KEY (`post_id`),
  KEY `idx_post_user_id` (`user_id`),
  KEY `idx_post_club_id` (`club_id`),
  KEY `idx_post_type` (`postType`),
  CONSTRAINT `Post_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `User` (`user_id`) ON DELETE CASCADE,
  CONSTRAINT `Post_ibfk_2` FOREIGN KEY (`club_id`) REFERENCES `Club` (`club_id`) ON DELETE CASCADE
